...
public class OrdenService {
    ...
    public Orden crear(Orden orden) {
        ...
        Orden ordenCreada = ordenRepository.save(creada);
        publicarSnapshotOrden(ordenCreada, false);
        return ordenCreada;
    }
    ...
    public Optional<Orden> actualizar(Integer id, Orden orden) {
        ...
        publicarSnapshotOrden(actualizada, true);
    ...
    public boolean eliminar(Integer id) {
        ...
        syncPublisher.publish("itemorden", "delete_by_orden", Map.of("idOrden", id));
        syncPublisher.publish("orden", "delete", Map.of("id", id));
    ...
    private void poblarItemsYTotalesDesdeCarrito(Orden orden, Carrito carrito) {
        ...
        for (ItemCarrito itemCarrito : carrito.getItems()) {
            ItemPricingService.ProductSubtotal calculo =
                    itemPricingService.calcularSubtotal(itemCarrito.getCodigoProducto(), itemCarrito.getCantidad());

            ItemOrden itemOrden = new ItemOrden();
            itemOrden.setOrden(orden);
            itemOrden.setCodigoProducto(calculo.codigoProducto());
    ...
    private void poblarItemsYTotalesDesdePayload(Orden orden, List<ItemOrden> itemsPayload) {
        ...
        for (ItemOrden itemPayload : itemsPayload) {
            ItemPricingService.ProductSubtotal calculo =
                    itemPricingService.calcularSubtotal(itemPayload.getCodigoProducto(), itemPayload.getCantidad());
    ...
    private void publicarSnapshotOrden(Orden orden, boolean reemplazarItems) {
        syncPublisher.publish("orden", "upsert", Map.of(
                "id", orden.getId(),
                "numero", orden.getNumero(),
                "idCarrito", orden.getCarrito().getId(),
        ...
        if (reemplazarItems) {
            syncPublisher.publish("itemorden", "delete_by_orden", Map.of("idOrden", orden.getId()));
        }

        for (ItemOrden item : orden.getItems()) {
            Map<String, Object> payload = new HashMap<>();
            payload.put("id", item.getId());
            payload.put("idOrden", orden.getId());
            ...
            syncPublisher.publish("itemorden", "upsert", payload);
        }
    }
}
